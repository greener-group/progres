...
with open(results_dir + "/eat_result.txt") as f:
    for l in f.readlines():
        if not l.startswith("Query-ID"):
            cols = l.split()
            domid_query, domid_match = cols[0], cols[2]
            if domid_query in domid_to_fam and domid_match in domid_to_fam:
                domid_to_fam_matches[domid_query].append(domid_to_fam[domid_match])
