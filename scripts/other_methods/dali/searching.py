...
pdbid_to_domid = {}
with open("domids_pdbids.txt") as f:
    for line in f.readlines():
        domid, pdbid = line.rstrip().split()
        pdbid_to_domid[pdbid] = domid

domid_to_fam_matches = defaultdict(list)
for domid_query in domids_unseen:
    fp = f"run/{domid_query}.txt"
    if domid_query in domid_to_fam and os.path.isfile(fp):
        with open(fp) as f:
            for line in f.readlines():
                if len(line.rstrip()) > 0 and not line.startswith("#"):
                    pdbid = line.strip().split()[1].split("-")[0]
                    domid_match = pdbid_to_domid[pdbid]
...
for di, domid in enumerate(domids_unseen):
    if domid not in domid_to_fam_matches:
        continue
